...
func NewKeyService(keyRepo keys.Repository, rd RedisClient, logger zerolog.Logger) *KeyService {
	...
	// Setup active key if it exists
	currentKey, err := keyRepo.GetActive(context.Background())
	if err != nil {
		logger.Info().Msg("Pre-active key not found, please generate a new key")
	} else {
		logger.Info().Msg("Pre-active key found, setting active key in Redis")
		rd.Set(context.Background(), "active_key", currentKey.Key, 0)
	}
...
func (s *KeyService) Get(ctx context.Context, cmd GetKeyCommand) (*GetKeyResult, error) {
	s.logger.Info().Msg("Getting key")
	key, err := s.rd.Get(ctx, "active_key").Result()
	if err != nil {
		s.logger.Error().Err(err).Msg("Error getting active key")
		return nil, err
	}
	s.logger.Info().Msg("Key retrieved")
	defer s.logger.Info().Msg("Key was successfully retrieved.")
	return &GetKeyResult{
		Key: key,
	}, nil
}
